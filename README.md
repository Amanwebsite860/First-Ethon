...
have **no code-level default** — both are required environment variables
(enforced at boot by `checkEnv.js`), and `.env.example` currently suggests
`gemini-3.5-flash-lite` for both. This is deliberate: an earlier version of
this app silently fell back to a hardcoded `gemini-2.5-flash` when the env
var was missing or failed to propagate on deploy, which masked a real
misconfiguration for a while (the app kept running, just against a model
that had been retired). Now a missing model var fails loudly at boot
instead. **Verify whatever model name you set is actually available to
